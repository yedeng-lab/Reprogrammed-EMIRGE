...
class EM(object):
    ...
    def __init__(self, reads1_filepath, reads2_filepath,
        ...
        self.cluster_thresh = cluster_thresh   # if two sequences evolve to be >= cluster_thresh identical (via usearch_64), then merge them. [0, 1.0]
    ...
    def do_iteration(self, bam_filename, reference_fasta_filename):
        ...
        self.cluster_sequences(consensus_filename)  # merges sequences that have evolved to be the same (USEARCH_64_64)
    ...
    def write_consensus_with_mask(self, reference_fastafilename, output_fastafilename, mask):
        ...
        this is useful prior to usearch_64 clustering.
    ...
    def cluster_sequences(self, fastafilename):
        """
        uses Edgar's USEARCH_64_64 to merge sequences above self.cluster_thresh %ID over the
    ...
    def cluster_sequences2(self, fastafilename):
        """
        uses USEARCH_64_64  to globally align sequences.  Merge two sequences if the
        *NON-GAPPED* positions have % identity >= self.cluster_thresh

        also adjusts Pr(S) [prior] and Pr(S_t-1) [posteriors] as needed after merging.

        only supports usearch_64 version > 6, as the command line substantially changed in this version.
        """
        if self._VERBOSE:
        ...
        # do global alignments with USEARCH_64_64/UCLUST.
        # I don't use --cluster because it doesn't report alignments
        # usearch_64 is fast but will sometimes miss things -- I've tried to tune params as best as I can.
        ...
        # there is a bug in usearch_64 threads that I can't figure out (fails with many threads).  Currently limiting to max 6 threads
        usearch_64_threads = min(6, self.n_cpus)
        cmd = "usearch_64 %s %s --db %s --id %.3f -quicksort -query_cov 0.5 -target_cov 0.5 -strand plus --userout %s.us.txt --userfields query+target+id+caln+qlo+qhi+tlo+thi -threads %d %s"%\
              (algorithm,
        ...
        if self._VERBOSE:
            sys.stderr.write("usearch_64 command was:\n%s\n"%(cmd))
        ...
        for row in csv.reader(file("%s.us.txt"%tmp_fastafilename), delimiter='\t'):
            ...
            if member_name == seed_name:
                continue # usearch_64 allows self-hits, which we don't care about
            ...
            #   0     1     2   3   4   5  6    7
            member_start = int(row[4]) - 1   # printed as 1-based by usearch_64 now
    ...
    def do_mapping_bowtie2(self, full_fasta_path, nice = None):
        ...
        if self.reads2_filepath is not None:
            bowtie2_command = " %s %s | %s bowtie2 %s -1 - -2 %s -x %s | python3 sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.PE.bam "%(\
                cat_cmd,
            ...
            bowtie2_command = " %s %s | %s bowtie2 %s -U - -x %s | python3 sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.PE.bam "%(\
                cat_cmd,
                self.reads1_filepath,
                nicestring,
                shared_bowtie2_params,
...
def do_initial_mapping(working_dir, options):
    ...
    if options.fastq_reads_2 is not None:
        ...
        cmd = " %s %s | %s bowtie2 --no-unal --phred%d -p %s -D %s -R %s -N %s -L %s -1 - -2 %s -x %s | python3 sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.bam "%tuple(option_strings)
    # SINGLE END MAPPING
    else:
        option_strings.extend([options.bowtie2_db, bampath_prefix])
        cmd = " %s %s | %s bowtie2 --no-unal --phred%d -p %s -D %s -R %s -N %s -L %s -U - -x %s | python3 sam_filter.py /dev/stdin /dev/stdout | samtools view -b -S -u -F 0x0004 - > %s.bam "%tuple(option_strings)
...
#     TODO: right now just checking usearch_64, as the command line params
...
#     match = re.search(r'usearch([^ ])* v([0-9]*)\.([0-9]*)\.([0-9]*)', Popen("usearch_64 --version", shell=True, stdout=PIPE).stdout.read())
#     if match is None:
#         print >> sys.stderr, "FATAL: usearch_64 not found in path!"
#         exit(0)
#     binary_name, usearch_64_major, usearch_64_minor, usearch_64_minor_minor = match.groups()
#     usearch_64_major = int(usearch_64_major)
#     usearch_64_minor = int(usearch_64_minor)
#     usearch_64_minor_minor = int(usearch_64_minor_minor)
#     if usearch_64_major < working_maj or \
#        (usearch_64_major == working_maj and (usearch_64_minor < working_minor or \
#                                           (usearch_64_minor == working_minor and usearch_64_minor_minor < working_minor_minor))):
#         print >> sys.stderr, "FATAL: usearch_64 version found was %s.%s.%s.\nemirge works with version >=  %s.%s.%s\nusearch_64 has different command line arguments and minor bugs in previous versions that can cause problems."%(usearch_64_major, usearch_64_minor, usearch_64_minor_minor, working_maj, working_minor, working_minor_minor)
#         exit(0)
#     return
